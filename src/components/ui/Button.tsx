...
const variantTokens: Record<ButtonVariant, Record<ButtonTone, {
  bg: string; color: string;
  hoverBg: string; hoverColor?: string;
  activeBg: string; activeColor?: string;
  disabledBg: string; disabledColor: string;
  border?: string; hoverBorder?: string; activeBorder?: string; disabledBorder?: string;
}>> = {
  Primary: {
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        ...
        if (v.hoverBorder) (e.currentTarget as HTMLButtonElement).style.border = v.hoverBorder;
      }}
      onMouseLeave={e => {
        if (disabled) return;
        (e.currentTarget as HTMLButtonElement).style.background = v.bg;
        if (v.border) (e.currentTarget as HTMLButtonElement).style.border = v.border;
        else (e.currentTarget as HTMLButtonElement).style.border = "none";
      }}
      onMouseDown={e => {
        if (disabled) return;
        (e.currentTarget as HTMLButtonElement).style.background = v.activeBg;
        if (v.activeColor) (e.currentTarget as HTMLButtonElement).style.color = v.activeColor;
        if (v.activeBorder) (e.currentTarget as HTMLButtonElement).style.border = v.activeBorder;
      }}
      onMouseUp={e => {
        if (disabled) return;
        (e.currentTarget as HTMLButtonElement).style.background = v.hoverBg;
        if (v.color) (e.currentTarget as HTMLButtonElement).style.color = v.color;
      }}
    >
      {leadingIcon && <span style={{ width: 24, height: 24, flexShrink: 0, display: "flex" }}>{leadingIcon}</span>}
      <span>{label}</span>
      {trailingIcon && <span style={{ width: 24, height: 24, flexShrink: 0, display: "flex" }}>{trailingIcon}</span>}
    </button>
  );
}
